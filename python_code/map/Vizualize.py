...
class Vizualize():
    ...
    def _draw_path(self):
        if len(self.map.path) > 0:
            last_point = self.map.path[0]
            for point in self.map.path[1:]:
                self.canvas.create_line(last_point.position[0] * self.scale + self.centre[0],
                                        last_point.position[1] * self.scale + self.centre[1],
                                        point.position[0] * self.scale + self.centre[0],
                                        point.position[1] * self.scale + self.centre[1],
                                        fill="green", width=2)
                self._draw_point(last_point)
                last_point = point
            self._draw_point(last_point)
